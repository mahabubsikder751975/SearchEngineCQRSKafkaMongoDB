...
namespace Search.Query.Infrastructure.Dispatchers
{
    ...
    public class QueryDispatcherSuggestion : IQueryDispatcher<SearchSuggestion>
    {
        ...
        private readonly Dictionary<Type, Func<BaseQuery, Task<List<SearchSuggestion>>>> _handlers = new();
        /// <summary>
        /// Register Handlers
        /// </summary>
        ...
        public void RegisterHandler<TQuery>(Func<TQuery, Task<List<SearchSuggestion>>> handler) where TQuery : BaseQuery
        {
        ...
        public async Task<List<SearchSuggestion>> SendAsync(BaseQuery query)
        {
            //Checking if it has query handler
            if (_handlers.TryGetValue(query.GetType(), out Func<BaseQuery, Task<List<SearchSuggestion>>> handler))
            {
