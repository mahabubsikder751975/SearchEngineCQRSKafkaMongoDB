...
namespace Search.Query.Infrastructure.Dispatchers
{
    ...
    public class QueryDispatcherSearchHistory : IQueryDispatcher<SearchHistory>
    {
        ...
        private readonly Dictionary<Type, Func<BaseQuery, Task<List<SearchHistory>>>> _handlers = new();
        /// <summary>
        /// Register Handlers
        /// </summary>
        ...
        public void RegisterHandler<TQuery>(Func<TQuery, Task<List<SearchHistory>>> handler) where TQuery : BaseQuery
        {
        ...
        public async Task<List<SearchHistory>> SendAsync(BaseQuery query)
        {
            //Checking if it has query handler
            if (_handlers.TryGetValue(query.GetType(), out Func<BaseQuery, Task<List<SearchHistory>>> handler))
            {
