...
namespace Search.Query.Infrastructure.Dispatchers
{
    ...
    public class QueryDispatcherContent : IQueryDispatcher<SearchContent>
    {
        ...
        private readonly Dictionary<Type, Func<BaseQuery, Task<List<SearchContent>>>> _handlers = new();
        /// <summary>
        /// Register Handlers
        /// </summary>
        ...
        public void RegisterHandler<TQuery>(Func<TQuery, Task<List<SearchContent>>> handler) where TQuery : BaseQuery
        {
        ...
        public async Task<List<SearchContent>> SendAsync(BaseQuery query)
        {
            //Checking if it has query handler
            if (_handlers.TryGetValue(query.GetType(), out Func<BaseQuery, Task<List<SearchContent>>> handler))
            {
