...
namespace Search.Query.Infrastructure.Dispatchers
{
    ...
    public class QueryDispatcherPlayList : IQueryDispatcher<PlayListItem>
    {
        ...
        private readonly Dictionary<Type, Func<BaseQuery, Task<List<PlayListItem>>>> _handlers = new();
        /// <summary>
        /// Register Handlers
        /// </summary>
        ...
        public void RegisterHandler<TQuery>(Func<TQuery, Task<List<PlayListItem>>> handler) where TQuery : BaseQuery
        {
        ...
        public async Task<List<PlayListItem>> SendAsync(BaseQuery query)
        {
            //Checking if it has query handler
            if (_handlers.TryGetValue(query.GetType(), out Func<BaseQuery, Task<List<PlayListItem>>> handler))
            {
